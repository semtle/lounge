...
function auth(data) {
	...
	if (client && data.token && client.config.sessions[data.token]) {
		client.config.sessions[data.token].lastUse = Date.now();

		authCallback(true);
		return;
	}

	// Perform password checking (either local or LDAP depending on configuration)
